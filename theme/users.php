...
?>
                                <div class="table-responsive m-t-40">
                                  <form id="RoleForm" autocomplete='off' action='javascript:roleChanger()'> 
...
if(isset($_GET['user'])){
	...
	if($conn->query($sql)!==TRUE)
		die("error in deleting user");
}
$sql="select u.name,u.username,u.role,u.age,u.nationality,m.num from users u left join (select username,count(*) as num from orders group by username) as m on u.username=m.username ";
$res=$conn->query($sql);
if($res->num_rows > 0){
	...
	while($row=$res->fetch_assoc()){
		...
		echo "<tr><td>".$row['name']."</td><td>
			...
			<option value='admin' "; if($row['role']=='admin') echo "selected='selected'"; echo ">Admin</option>
			<option value='customer'"; if($row['role']=='customer') echo "selected='selected'"; echo ">Customer</option>
			</select>
			</td><td>".$row['nationality']."</td><td>".$row['age']."</td><td>".$row['num']."</td><td><a href='users.php?user={$row['username']}'>Delete</a></tr>";
	}
	echo "</tbody></table>
...
function roleChanger(){
	...
	ajax.open("GET",'../back/role.php?'+$('#RoleForm').serialize(),true);
	ajax.send();
}
</script>
